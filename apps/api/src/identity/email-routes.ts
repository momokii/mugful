...
export const registerIdentityEmailRoutes = (
  ...
  app.post(
    ...
    { schema: identityOpenApiSchemas.resendVerification },
    async (request, reply) => {
  ...
  app.post(
    ...
    { schema: identityOpenApiSchemas.confirmVerification },
    async (request, reply) => {
      if (!dependencies.verifyUnsafeRequest(request))
        return dependencies.sendForbidden(reply);
      ...
      const result =
        await dependencies.identityEmailService.confirmVerification(
          parsed.data.token,
        );
  ...
  app.post(
    ...
    { schema: identityOpenApiSchemas.forgotPassword },
    async (request, reply) => {
      if (!dependencies.verifyUnsafeRequest(request))
        return dependencies.sendForbidden(reply);
      const parsed = emailSchema.safeParse(request.body);
      if (!parsed.success)
        return reply.code(400).send({ error: "invalid email" });
      const result =
        await dependencies.identityEmailService.requestPasswordReset(
          parsed.data.email,
        );
      return result === "rate-limited"
        ? sendRateLimited(reply)
  ...
  app.post(
    ...
    { schema: identityOpenApiSchemas.resetPassword },
    async (request, reply) => {
      if (!dependencies.verifyUnsafeRequest(request))
        return dependencies.sendForbidden(reply);
