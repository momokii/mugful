...
Run the Todo 6 `/onboarding` and `/join` Chromium matrix once a supported browser runtime is installed, then perform independent final review before accepting Todo 6. Do not add privacy-request operations, video, AI, analytics, Redis, or a second activity while building v1. Team-mode security review remains unavailable.
