...
- Todo 5A persists accounts, independently versioned adult-attestation/terms/privacy consent records, sessions, and email-verification/password-reset token metadata. The Todo 5B2 registration transaction creates the account and all three required consent records atomically.
...
Todo 5B2 exposes the internal `/v1` identity checkpoint: CSRF issuance; config-gated registration; verified-account login; logout; current session; password change; active-device listing; and owned non-current device revocation. Registration accepts only three separately affirmative values for adult self-attestation, Terms, and Privacy Notice; the server alone selects and atomically persists the current `adult-v1`, `terms-v1`, and `privacy-v1` records. Todo 5C adds verification resend/confirm and password forgot/reset commands. Todo 5E publishes the implemented HTTP contract at `/openapi.json` only; it does not install or expose Swagger UI. The implemented Todo 5F foundation adds authenticated `GET /v1/auth/privacy`, a read-only summary of the caller's active Terms/Privacy versions and acceptance timestamps plus email-verification status. It does not add privacy export, correction, deletion, withdrawal, or restriction operations. Every unsafe identity command requires an exact same-origin `Origin` header plus a signed CSRF token bound to a browser CSRF cookie. Login, registration, and email requests use durable PostgreSQL rate-limit buckets keyed only by an HMAC of their normalized principal. Generic credential, registration, and absent-email responses avoid account enumeration. Verification and reset tokens are random, HMAC-persisted, replaced when reissued, atomically consumed, and sent only in URL fragments. Token/account state commits before mail delivery; failed delivery remains generic and can be retried. Password reset invalidates every account session without issuing one; verification does not create a session. Migrations remain explicit manual operations; startup and health boundaries do not execute them.

The implemented `/verify-email`, `/reset-password`, and `/join` UI pages read tokens from fragments only and set `Referrer-Policy: no-referrer` plus `Cache-Control: no-store`. Todo 6 adds `/v1/couple-space`, `/v1/couple-invites/accept`, and `/v1/couple-space/end`: verified authenticated sessions and same-origin CSRF are required for every mutation. PostgreSQL enforces one active membership per account; create and accept lock the account before checking membership, invite tokens are HMAC-persisted only, expire after seven days, and are returned once exclusively in a `/join#token=…` fragment URL. Redemption locks the invite and space, is single-use, rejects the creator and already-coupled accounts, then revokes any remaining pending invite. Ending revokes all active memberships and pending invites immediately while recording a 30-day deletion grace end time.

Session-cookie policy is deployment-specific and not caller-configurable: the `WEB_ORIGIN` scheme selects it, independently of `NODE_ENV`. An `https:` origin uses `__Host-mugful-session` and `__Host-mugful-csrf` with `Secure`, `HttpOnly`, `SameSite=Lax`, `Path=/`, and no `Domain`; an `http:` origin uses plain `mugful-session` and `mugful-csrf` cookies without `Secure` for trusted Tailscale/LAN access. `WEB_ORIGIN` must exactly match the browser origin, including scheme, host, and port, because every unsafe identity command checks that `Origin` header and a signed CSRF token. The browser CSRF cookie shares the session lifetime: `session-store.ts` exports the single 30-day `sessionLifetimeSeconds` constant, stored session expiry and the CSRF cookie `Max-Age` both derive from it, and integration tests pin the two `Max-Age` values together so a logged-in session can never outlive its CSRF cookie.

Protected web routes use a server-side session-gate seam that forwards browser cookies to the API's internal session endpoint and redirects unauthenticated requests. This is deliberately not Next middleware: standalone Next builds inline server environment configuration, while the API's internal origin is a runtime Compose value. Keeping the gate in server-rendered routes preserves that runtime configuration boundary.
