...
The implemented `/verify-email` and `/reset-password` UI pages read tokens from fragments only and set `Referrer-Policy: no-referrer` plus `Cache-Control: no-store`.
